    ...
    public class AccountController : Controller
    {
            ...
            switch (result)
            {
                case SignInStatus.Success:
                    ...
                    if (!UserManager.IsEmailConfirmed(userid))
                    {
                        ...
                        LogOff();
                        return View("NotConfirmEmail",model);
                    }
                    else
                    {
        ...
        public async Task<ActionResult> Register(RegisterViewModel model)
        {
            ...
            if (ModelState.IsValid)
            {
                ...
                if (result.Succeeded)
                {
                    var userU = new Uzytkownik();
                    userU.AdresEmail = model.Email;
                    userU.Imie = model.Imie;
                    userU.Nazwisko = model.Nazwisko;
                    userU.DataUrodzenia = model.DataUrodzenia;
                    dbo.Uzytkownik.Add(userU);
                    dbo.SaveChanges();
                    user.id_uzytkownik = userU.id;                                       
                    AspNetUsers tmpU = dbo.AspNetUsers.Where(u=>u.Id == user.Id).FirstOrDefault();
                    tmpU.id_uzytkownik = userU.id;
                    dbo.Entry(tmpU).State = EntityState.Modified;
                    dbo.SaveChanges();
        ...
        [AllowAnonymous]
        [HttpPost]
        public async Task<ActionResult> RepeatConfirmEmail(LoginViewModel model)
        {
            
            var userid = UserManager.FindByEmail(model.Email).Id;           
            string code = await UserManager.GenerateEmailConfirmationTokenAsync(userid);
            var callbackUrl = Url.Action("ConfirmEmail", "Account", new { userId = userid, code = code }, protocol: Request.Url.Scheme);
            await UserManager.SendEmailAsync(userid, "Potwierdź konto", "Potwierdź konto, klikając <a href=\"" + callbackUrl + "\">tutaj</a>");
